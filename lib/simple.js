...
END:VCARD
		...
		sendButtons: {
			/**
			 * Send native flow buttons via ourin-baileys interactiveMessage
			 * @param {String} jid
			 * @param {String} text main body text (mapped to title)
			 * @param {Array} buttons list of button objects:
			 *   quick reply (default): { id, title }
			 *   url button: { type: 'url', title, url }
			 *   copy button: { type: 'copy', title, code }
			 *   list menu: { type: 'list', title, sections: [{ title, rows: [{ title, id, description }] }] }
			 *   taps come back as m.text containing the id (handled by smsg parser)
			 * @param {Object} options { footer, header, image, video, quoted, mentions, ... }
			 */
			async value(jid, text = '', buttons = [], options = {}) {
				const { footer = '', header, quoted, mentions, ...rest } = options;
				const nativeButtons = (buttons || []).map((b) => {
					if (b.type === 'url') return { name: 'cta_url', buttonParamsJson: JSON.stringify({ display_text: b.title, url: b.url }) };
					if (b.type === 'copy') return { name: 'cta_copy', buttonParamsJson: JSON.stringify({ display_text: b.title, copy_code: b.code }) };
					if (b.type === 'list') return { name: 'single_select', buttonParamsJson: JSON.stringify({ title: b.title || 'Menu', sections: b.sections || [] }) };
					return { name: 'quick_reply', buttonParamsJson: JSON.stringify({ display_text: b.title || b.id, id: b.id || b.title }) };
				});
				const interactiveMessage = { title: text, footer, buttons: nativeButtons, ...rest };
				if (header) interactiveMessage.header = header;
				// Mention di interactive message wajib lewat contextInfo.mentionedJid
				// (bukan field top-level) agar @tag biru & masuk notifikasi.
				if (mentions?.length) interactiveMessage.contextInfo = { ...(interactiveMessage.contextInfo || {}), mentionedJid: mentions };
				return conn.sendMessage(jid, { interactiveMessage }, quoted ? { quoted } : {});
			},
			enumerable: true,
		},
		cMod: {
